...
void MainGame::initZombies() {
	GLuint zombieTexture = ResourceManager::getTexture("path_to_zombie_texture.png").id;
	...
	for (int i = 0; i < 2; ++i) {
		...
		zombies.back()->init(2.0f, glm::vec2(x, y), zombieTexture);
	}
}
...
void MainGame::run() {
	...
	while (gameState == GameState::PLAY) {
		player->update(levelData, humans, zombies);

		// Actualizar los zombis
		for (auto& zombie : zombies) {
			zombie->update(levelData, humans, zombies);
		}

		// Dibujar el jugador y los zombis
		spriteBatch.begin();
		player->draw(spriteBatch);
		for (auto& zombie : zombies) {
